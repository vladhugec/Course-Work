...
(* [*]                                          *)
(*                                              *)
(* Printing types and constraints               *)
(*                                              *)
...
and (* At a mathematical level, \nml and micro-Scheme have *)
    ...
    (* because it has [[set]], environments map names to *)
    (* mutable cells. In \nml, environments map names to *)
    (* values. How do we implement recursive functions? The *)
    (* value of a function is a closure, and to make a *)
    (* recursive call possible, the closure must contain the *)
    (* value of the function. To implement a closure that *)
    (* refers to itself in this way, we need some sort of *)
    (* cyclic representation. In my implementation of *)
    (* micro-Scheme, I use mutation to create a graph with a *)
    (* cycle. In my implementation of \nml, I build each *)
    (* closure's environment lazily; that is, instead of *)
    (* storing an environment in the closure, I store a *)
...
(* Except for [[VALREC]], definitions are as in *)
(* micro-Scheme. \nmllabeldef                   *)
      ...
      val primBasis = foldl addPrim (emptyTypeEnv, emptyEnv) 
                     ...
                     (* normally. In \nml this type means it must either halt *)
                      ...
                      (* function of this type could also raise an exception. *)
                          ...
                          (* The list primitives appear only in \nml; in uML, *)
                          (* a list is an algebraic data type, and the list *)
                          (* operations are written in user code.         *)
                          (* <primitives for \nml\ [[::]]>=               *)
                          ...
                          [])
      val usercode =
         (* [*] In Typed uScheme, change the code in \chunkref *)
         (* tuscheme.chunk.capture-avoiding-substitution so that *)
         (* substitution is always done naï vely, in a way that *)
         (* allows the capture of a \/-bound variable.   *)
         (* (It suffices to write \monoboxif true instead of \ *)
         (* monoboxif null [[actual_captures]].)         *)
         (*                                              *)
         (*  1. Define a typed, polymorphic version of function *)
         (*  [[flip-apply]] from \cpageref               *)
         (*  tuscheme.code.flip-apply, which should have type *)
         (*  \/beta\alldotbeta-->( \/alpha\alldot(beta-->alpha *)
         (*  ) --> alpha).                               *)
         (*  2. Using [[type-lambda]] and suitable instantiations *)
         (*  of [[flip-apply]], define a function [[cast]] *)
         (*  that behaves like the identity function but has *)
         (*  type \/alpha, beta\alldotalpha-->beta.      *)
         (*  This function is not legitimate; in a correct *)
         (*  version of Typed uScheme, it doesn't typecheck. *)
         (*  But if you allow variable capture, you can write *)
         (*  it.                                         *)
         (*  3. Use [[cast]] to introduce a variable that has *)
         (*  type [[int]] but value [[#t]].              *)
         (*  4. Trigger [[BugInTypeChecking]] by adding one to *)
         (*  this variable.                              *)
         (*                                              *)
         (* \readychapterml                              *)
         (*                                              *)
         (* <predefined {\nml} functions, as strings>=   *)
