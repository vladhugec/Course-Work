...
/* eval.c 45a */
Value eval(Exp e, Valenv globals, Funenv functions, Valenv 
                                                formals, Valenv locals) {
...
/* make [[f]] the function denoted by [[e->u.apply.name]], 
or call [[runerror]] 48c */
            ...
            case USERDEF:
                /* apply [[f.u.userdef]] and return the result 49b */
                {
                    ...
                    Valuelist lvs = NULL; //local-valuelist
                    
                    //initialize locals with value 0
                    while (ls != NULL) {
                        lvs = mkVL(0, lvs);
                        ls = ls->tl;
                    }
                    
                    //reset locals to start of list
                    ls = f.u.userdef.locals;
                    
                    checkargc(e, lengthNL(xs), lengthVL(vs));
                    checkargc(e, lengthNL(ls), lengthVL(lvs));
                    
                    Valenv formals = mkValenv(xs, vs);
                    Valenv locals = mkValenv(ls, lvs);
                    
                    return eval(f.u.userdef.body, globals, functions, formals, 
                                                                      locals);
...
/* check that [[vs]] has exactly two values,
 and assign them to [[v]] and [[w]] 51c */
