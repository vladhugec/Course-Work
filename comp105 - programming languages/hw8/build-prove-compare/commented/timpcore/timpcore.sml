...
(* Typed Impcore by presenting the representations I use *)
...
(* Appendix [->] defines functions used to print types. *)
(* <boxed values 79>=                           *)
...
(* risk introducing bugs that will be very hard to find. *)
...
(* What little more we need is defined elsewhere: the *)
(* definition of [[xdef]] is shared with micro-Scheme, *)
(* and functions [[valueString]] and [[expString]] are *)
(* defined in \crefapp:typesys.                 *)
...
(* explicitly. By analogy with C, the concrete syntax *)
(* puts types on the left. But as is customary in formal *)
(* semantics and in Pascal-like and ML-like languages, *)
(* the abstract syntax puts types on the right. \ *)
(* timplabeldef                                 *)
...
fun typeof (e, globals, functions, formals) =
  let (* All literals are integers. \usetyLiteral     *)
   ...
   (* <function [[ty]], to check the type of an expression, given $\itenvs$>= *)
      ...
      (* these rules, I call [[ty]] recursively to determine *)
      (* the types of the variable and the expression, then *)
      (* compare for equality. If they're not equal—a case not *)
      (* covered by the specification—the implementation gives *)
      (* an explanatory error message, which is more work than *)
      (* checking the types. One benefit of theory is that *)
      (* theorists can ignore that stuff.             *)
...
(* Here are two functions that compare types for *)
(* equality.                                    *)
(* <boxed values 80>=                           *)
...
fun elabdef (d, globals, functions) =
    case d
      of (* <cases for elaborating definitions in \timpcore>= *)
         ...
         (* I compute this value using [[find]] and an exception *)
         (* handler. Once I have [[x's_type]], sharing the common *)
         (* parts of the two rules is not too hard. [*]  *)
         (* <boxed values 83>=                           *)
         ...
         (* function's body is more elaborate than the checking *)
         (* of a [[val]] binding's right-hand side: as shown in *)
         (* both rules, before I check the body, I put the type *)
         (* of [[f]] into the type environment [[functions']]. *)
         (* Once checked, the body's type [[tau]] has to match *)
         (* the type [[returns]] given in the definition, and if *)
         (* [[f]] is already defined in the [[functions]] *)
