...
(* <validation by case analysis on [[schemeShape shape]] and [[k]] ((existentials))>= *)
...
(* <function [[ty]], to infer the type of a \nml\ expression, given [[Gamma]]>= *)
      fun ty (LITERAL n) = literal n
        ...
        (* <more alternatives for [[ty]]>=              *)
        | ty (CASE (e, choices)) = 
            let (* Finally, function [[pvconType]] implements the *)
                (* judgment \typeispatK tau                     *)
                ...
                fun choicetype ((p, e), Gamma) =
                      let val (Gamma', tau, c) = pattype (p, Gamma)
                          ...
                          val _ =
                      (* We're not quite done with skolem types. Skolem types *)

                              (* don't just look different from ordinary type *)

                       (* constructors; they are also semantically different. *)

                     (* An ordinary type constructor like [[int]] or [[bool]] *)

                      (* always means the same set of values at run time. But *)

                       (* a skolem type that appears in a case expression can *)

                        (* mean something different on each evaluation of the *)

                           (* case expression. Just think about the [[shape]] *)

                         (* functions above. In [[scale-shape]], for example, *)

                        (* sometimes the hidden type is [[circle]], but other *)

                       (* times it is \monobox(list pt). But within the scope *)

                              (* of the case expression, both of these hidden *)

                     (* representations are given the same skolem type, say \ *)

                      (* skolemty_17. It is absolutely crucial that \skolemty *)

                        (* _17 not escape the case expression. That's because *)

                     (* the equivalence \skolemty_17 ===\skolemty_17 is sound *)

                         (* only for duration of a single evaluation. We must *)

                              (* prevent all the following means of escape:   *)

                              (*                                              *)

                    (*   • A skolem type appears in the type of the result. *)

                            (*   • A skolem type appears in the type of the *)

                              (*  scrutinee.                                  *)

                     (*   • A skolem type appears in a constraint in such a *)

                           (*  way that it wants to be substituted for a type *)

                           (*  variable that appears free in the environment. *)

                              (*                                              *)

                      (* So the skolem types that are introduced by a pattern *)

                      (* match must not appear in either the argument type or *)

                              (* the result type of that pattern match.       *)

                              (*                                              *)

                            (* \tyruleExistentialChoice \multiline \twoquad \ *)

                      (* pattypeisc\tycp Gamma' tau \typeisc[+Gamma'] \tyc' e *)

                         (* tau' \subsn(C \landC') ===\trivc \multiquad \fs(\ *)

                       (* subsnGamma') \cap\fs(\subsnGamma) = \emptyset \fs(\ *)

                      (* subsnGamma') \cap\fs(\subsn(tau-->tau')) = \emptyset *)

                     (* \typeisc\tyc\land\tyc' \choicep e tau-->tau' The rule *)

                              (* is implemented using these representations:  *)
