...
TestResult test_result(UnitTest t, Env rho) {
    ...
    case CHECK_EXPECT:
        /*
         * Aside from the environment, there is one other
         * difference between the micro-Scheme [[check-expect]]
         * and the Impcore [[check-expect]]. In Impcore, values
         * are integers, and we test for inequality using C's
         * [[!=]] operator. In micro-Scheme, values are
         * S-expressions, and we test for equality using
         * C function [[equalpairs]] (defined below), which
         * works the same way as the micro-Scheme function
         * [[equal?]].
