...
/*
 * Function [[getline_]] returns a pointer to the next
 * line from the input, which is held in [[buf]], a
 * buffer that is reused on subsequent calls. Function
 * [[growbuf]] makes sure the buffer is at least [[n]]
 * bytes long.
 * <linestream.c>=
 */
...
/*
 * Here's a hidden trick: I've tweaked [[getline_]] to
 * check and see if the line read begins with the
 * special string [[;#]]. If so, the line is printed.
 * This string is a special comment that helps me test
 * all the \LAtranscript\RA examples in the book. \
 * implabelgetline_
 * <linestream.c>=
 */
char* getline_(Linestream lines, const char *prompt) {
    ...
    if (lines->fin)
        /*
         * To get a line from a file, I call the C standard
         * library function [[fgets]]. If the buffer is big
         * enough, [[fgets]] returns exactly the next line.
         * If the buffer isn't big enough, I grow the buffer and
         * call [[fgets]] again, to get more of the line.
         * This process iterates until the last character in the
         * buffer is a newline. I then chop off the newline by
        ...
        /*
         * When reading from a string, I look in [[lines->s]].
         * I find the next newline, copy the characters into
         * [[buf]], and update [[lines->s]].
