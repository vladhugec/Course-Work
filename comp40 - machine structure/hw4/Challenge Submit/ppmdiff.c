...
float computeMeanSqDiff(Pnm_ppm pmF1, Pnm_ppm pmF2, fComp *fData) {
        ...
        for (int w = 0; w < fData->smallerWidth; w++) {
                for (int h = 0; h < fData->smallerHeight; h++) {
                        ...
                        float red1 = pixF1->red / 255;
                        float green1 = pixF1->green / 255;
                        float blue1 = pixF1->blue / 255;

                        float red2 = pixF2->red / 255;
                        float green2 = pixF2->green / 255;
                        float blue2 = pixF2->blue / 255;

                        double redDiffSq = pow(abs(red1 - red2), 2);
                        double greenDiffSq = pow(abs(green1 - green2), 2);
                        double blueDiffSq = pow(abs(blue1 - blue2), 2);
