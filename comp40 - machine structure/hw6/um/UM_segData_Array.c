...
UM_data make_UM_data()
{
        ...
        UM->segments = malloc(sizeof(SIZE*)*100000);/*start w/ room for 100 segs*/
...
void initialize_UM_data(FILE *fp, UM_data data, SIZE numBytes)
{
        (data->segments)[0] = malloc(numBytes);
        ...
        for (SIZE j = 1; j < 100000; j++){
                Seq_addlo(data->unmappedSegs, (void*)(uintptr_t)j);
        }
        
        data->pCounter = 0;
}

void UM_data_free(UM_data *data, SIZE numBytes)
{
        (void)numBytes;
        for (unsigned i = 0; i < 100000; i++) {
                free(((*data)->segments)[i]);
        }
...
/* 
uint32_t* UM_data_makeTOarray(UM_data data)
{
        ...
        uint32_t* zero_seg = malloc(sizeof(uint32_t) * numVals);
        
        for (int i = 0; i < numVals; i++) {
                zero_seg[i] = ((uint32_t)(uintptr_t)Seq_get(targetSeg, i));
        }
...
SIZE UM_data_get_pCounter(UM_data data) {
        return data->pCounter;
}

SIZE* UM_data_get_regs(UM_data data)
{
...
SIZE UM_data_get_instruction(UM_data data, 
                                uint32_t outer_i, uint32_t inner_i)
{
        return (data->segments)[outer_i][inner_i];
}
/* 
...
int UM_data_get_seg_instruction_count(UM_data data, SIZE outer_i)
{       
        return (sizeof(*(data->segments[outer_i]))/sizeof(SIZE));
}

SIZE UM_data_get_next_unmapped_loc(UM_data data)
{
...
void UM_data_seg_put(UM_data data, SIZE* new_seg, SIZE loc)
{
        (data->segments)[loc] = new_seg;
}
/* 
                                ...
                                SIZE outer_i, SIZE inner_i)
{
        if (outer_i != 0) {
                data->sizeZeroSeg = UM_data_get_seg_instruction_count(data, 
                                                                outer_i);
                SIZE* new_seg = malloc(4*(data->sizeZeroSeg));

                for (SIZE i = 0; i < data->sizeZeroSeg; i ++) {
                        new_seg[i] = (data->segments)[outer_i][i];
                }
                
                free((data->segments)[0]);
                (data->segments)[0] = new_seg;
        }
...
{
        (data->segments)[outer_i][inner_i] = val;
}
